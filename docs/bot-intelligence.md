...
navigation, and stateless decision inputs let the production authority and local
fixed-step simulation run several bots without duplicating world scans. Larger
rosters, team coordination, and persistent learning are separate projects.
...
The planner executes only from the ordered 60 Hz canonical `FixedUpdate` schedule
and advances its integer decision clock at exactly 20 Hz. Authority-private plan
memory never enters a predicted browser client's rollback snapshot; the authority
commits the resulting ordinary `InputFrame` values to the same bounded input tape
used for human seats. Replaying that tape therefore reproduces the canonical match
without running the planner on the client.
...
Non-shipping native development builds poll for changes every 0.5 seconds. A
missing or invalid development file uses compiled defaults; a failed reload
retains the last valid catalog. Shipping, browser, renderless authority, and
deterministic fixture builds always parse the immutable embedded catalog. Each bot
copies its normalized profile at match or behavior start, so development hot reload
never changes an in-progress match.
...
the authoritative bot hot path also require same-hardware before/after
`afc-multiplayer-profile` captures under the protocol in `docs/performance.md`.
The profiler independently gates planner p95 below 0.1 ms, the complete authority
step p99 below 1 ms, and zero steady-state authority allocations. Web builds are
