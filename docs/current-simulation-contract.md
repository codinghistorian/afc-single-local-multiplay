...
Champion's Court now uses the final authored 35-barrier topology (18 rectangles,
17 circles, including one one-way top) instead of the earlier 91-record layout.
Its exact runtime words are frozen by FNV-1a fingerprint
`c5499906dcd78474`. BF021 intentionally changes from two fighters running along
the old lateral exits to one fighter traversing the final front apron. The
winner remains team 1, while the deciding tick changes from 709 to 934. BF024,
BF025, and BF028 relocate their synthetic setups clear of the final collision
without changing their contested-pickup, simultaneous-respawn, or item-lifecycle
semantics.

Split Causeway uses 18 exact-bit static barriers with fingerprint
`48cbea118f23e9bc`; gate leaves are excluded from that table and have their own
19-pose (closed through 18 ticks open) collision fixture. Gate eligibility is
recomputed from canonical fighter pose/action, with equal candidates resolved by
`FighterId`. An eligible light press is consumed in stable fighter order and
emits exactly one `ArenaDeviceToggled { arena_index, device_index, active }`
canonical event. Each gate advances at most once per fixed tick, stops before
overlapping an active fighter, and drives presentation from the same hinge
transform used by canonical collision. Prompts, labels, audio, and mesh motion
remain presentation-only.

BF031 freezes the first gate's stable event at tick 1, progress 1 through 18,
the fully open state thereafter, and restore/replay from tick 10. All 20 tapes
match across clean, restored, and presentation-perturbed executions. Debug and
fat-LTO release regenerated byte-identical fixture corpora with BF001 tick-1
hash `3eae3ee94c4516d7`; the compiled gameplay-content digest is
`11f250ab9cc50f8caee1cb34f7cb387c474996b68db84535f4d07b688b214e03`.
The production stock tape and eleven-arena matrix are frozen in
[cross-platform-determinism.md](cross-platform-determinism.md). This accepted
arena behavior and schema change is approved by the browser-multiplayer
integration scope.

## Simulation v7 retired shared-special addendum

At the simulation-v7 compatibility boundary, protocol version 1, replay schema
1, snapshot schema 3, and the 60 Hz fixed-tick schedule were unchanged. The
shared Pulse Dart / Trip Plate / Snap Wave / Drift Field
...
| `SplitCausewayDoorState` | two stable authored gate target flags and 0-18 progress ticks; eligible fighter prompts are recomputed and excluded | Canonical logical/motion state in snapshot-schema-4 arena payload; indexed by immutable device ID |
