...
| `MapCycle100` | Preload exactly 113 supported assets from `arena`, `backgrounds`, and `music/bgm`; present one complete eleven-arena warm precycle, then measure 100 presented switches. | Exactly 100 switch samples, 112 present acknowledgements including initial/final fences, 10 aligned cycle checkpoints, exact scene/item/hazard counts, and aligned RSS/live-byte gates. |
...
For `MapCycle100`, executable validity requires the exact 11-present warm
precycle, 100 measured switch/latency samples, 112 total present
acknowledgements, and 10 aligned cycle checkpoints. Resource counts must match
...
| --- | --- | --- |
| Authority 60 Hz step p99 below 1 ms | `afc-multiplayer-profile` production headless authority sample | Current simulation-v10 developer reference accepted at 874,458 ns median-of-three p99; minimum-supported-CPU capture remains pending |
| Tactical bot planner p95 below 0.1 ms | Per-invocation timing inside the measured production authority sample | Current developer reference accepted at 36,916 ns median-of-three p95 |
| Exact 12-tick rollback p99 below 4 ms | `afc-multiplayer-profile` production correction/resimulation sample | Current simulation-v10 developer reference accepted at 405,791 ns median-of-three p99 and exact depth 12; minimum-supported-CPU capture remains pending |
| Zero steady-state simulation allocations | Counting-allocator deltas around each authority sample | Current simulation-v10 developer reference accepted at zero allocations in all three 1,000-step runs |
...
| 2026-07-27 | Apple M2 Max, macOS | Corrected itch.io web release | Optimized game WASM 44,263,596 bytes; `web_dist/` 103,509,569 extracted bytes across 214 files; ZIP 64,575,622 bytes. | Historical corrected size/package baseline after pinning the `ffc-prototype` browser-game binary; a current integration-branch build and Chrome/Safari runtime measurements remain pending. |
...
| 2026-08-27 | Mac14,6, Apple M2 Max (12-core CPU, 32 GiB), macOS 26.5.1 (25F80), AC power, `pmset powermode=2`, no fixed affinity | Simulation-v10 Chick stamina-cost integration, immutable pre-`c9a6696`/post `afc-multiplayer-profile` executables, three alternating 1,000-sample pairs ([raw JSON](performance-evidence/2026-08-27-chick-stamina.jsonl)) | Median p99 changed from 868,750 to 874,458 ns for authority (+0.7%) and 407,083 to 405,791 ns for exact 12-tick rollback (-0.3%). Bot-planner median p95 changed from 37,292 to 36,916 ns (-1.0%). All six captures passed; authority stayed allocation-free, rollback diagnostics remained 118,586 allocations / 142,135,996 bytes, and depth/history gates stayed at 12 and 128/64/64. Executable SHA-256 values were `99d3c46f04a7dda3cb84e3b650621ba0cce5aa8644277b06dc76947750a24d90` before and `8b5ecfd1703445a0916990a758ed8c09aff21c57fdc67a7c2cf757ae340f4502` after. | Accepted current same-hardware functional baseline. The change is immaterial, preserves all gates, and leaves 12.6% authority-p99 headroom on this developer machine; minimum-supported-CPU evidence remains required. |
| 2026-08-28 | Mac14,6, Apple M2 Max (12-core CPU, 32 GiB), macOS 26.5.1 (25F80), battery power, `pmset powermode=2`, no fixed affinity | Landing-linger snapshot-envelope correction, mechanically isolated pre/post `afc-multiplayer-profile` executables, three sequential 1,000-sample runs per condition ([raw JSON](performance-evidence/2026-08-28-browser-linger-snapshot.jsonl)) | Median p99 changed from 869,709 to 870,417 ns for authority (+0.1%) and 397,042 to 392,375 ns for exact 12-tick rollback (-1.2%). Bot-planner median p95 changed from 36,667 to 36,834 ns (+0.5%). All six captures passed; authority stayed allocation-free, rollback diagnostics remained 118,586 allocations / 142,135,996 bytes, and depth/history gates stayed at 12 and 128/64/64. | Accepted same-hardware preservation evidence for a correctness fix, not an optimization claim. The changes are immaterial and the simulation-v10 developer baseline remains accepted; minimum-supported-CPU evidence remains required. |
| 2026-08-28 | Mac14,6, Apple M2 Max, macOS 26.5.1, Google Chrome | Browser multiplayer release candidate `qa-browser-20260828-7`; optimized itch.io package and real two-, three-, and four-client battle/rematch acceptance | Optimized WASM 46,770,722 bytes (SHA-256 `7ac3a18391b413914db9e957f3371403d4db6a295777f461a5cebaddaa1ae7a9`); `web_dist/` 138,233,666 extracted bytes across 237 files; ZIP 97,618,426 bytes (SHA-256 `b5d15a8973665b463f9a29ec639509528950ee83c95e745e2693cc3b68473ea3`). All three client-count runs completed a battle and rematch, confirmed two distinct authority results, and returned every client to the same room; maximum authority-confirmation lag was one tick and all failed-room/adapter-error counters remained zero. | Accepted local Chrome functional/package evidence. The optimized WASM remains below the 50 MiB budget. This does not replace the pending Chrome/Safari frame-time or deployed trusted-TLS evidence. |
...
### Simulation-v6 manual-aim preservation capture

This comparison covers the fixed-tick manual-aim target selection and its
schema-v3 rollback extension. The detached before source was the accepted
simulation-v5 tutorial batch at `f5110c7`; the after executable contains the
canonical stable-ID aim state while marker placement remains presentation-only.
Both immutable arm64 executables used rustc 1.94.1, the profiling profile, seed
`0x00000000ffc00001`, 256 authority warmup ticks, 16 rollback warmup bursts,
1,000 timed samples, and exact rollback depth 12. The machine remained on AC
power under High Power mode with no fixed affinity.
...
| Pair | Before authority p99 (ns) | After authority p99 (ns) | Before rollback p99 (ns) | After rollback p99 (ns) |
| ---: | ---: | ---: | ---: | ---: |
| 1 | 67,667 | 65,667 | 376,666 | 375,167 |
| 2 | 56,166 | 55,917 | 366,083 | 385,542 |
| 3 | 54,375 | 55,208 | 369,458 | 370,250 |
| Median | 56,166 | 55,917 | 369,458 | 375,167 |
...
and rollback snapshot/input high-water 64/64. The immutable before executable
SHA-256 is
`c3c8f0dc233a44c8709515ab081fba5ad54fe20111bdb8a634346ef3abe94055`;
the after executable SHA-256 is
`c185f42020c24909e8b353f2fbc3b4300343ef98c774fa464febfa0439a3c9e0`.
All six raw result lines are retained under
`target/perf-captures/manual-aim/results/`.

### Simulation-v7 retired shared-special preservation capture

This comparison covers the fixed-tick and authoritative gates that retire the
shared special action while preserving its legacy wire bit. The immutable before
source is simulation-v6 commit `e0df740`; the immutable after source is
simulation-v7 commit `cce7984`. Both arm64 executables used rustc 1.94.1, the
profiling profile, seed `0x00000000ffc00001`, 256 authority warmup ticks, 16
rollback warmup bursts, 1,000 timed samples, and exact rollback depth 12. The
machine remained on battery power under High Power mode with no fixed affinity;
the official alternating set stayed at 92% battery at every recorded edge.

| Pair | Before authority p99 (ns) | After authority p99 (ns) | Before rollback p99 (ns) | After rollback p99 (ns) |
| ---: | ---: | ---: | ---: | ---: |
| 1 | 83,750 | 66,292 | 437,084 | 448,917 |
| 2 | 67,083 | 62,500 | 437,250 | 466,750 |
| 3 | 58,250 | 55,042 | 393,500 | 380,959 |
| Median | 67,083 | 62,500 | 437,084 | 448,917 |

Authority p99 changed by -6.8% and exact rollback p99 by +2.7%. This is
preservation evidence for a functional workload change: the authority no longer
executes shared-special spawn/cooldown work, so the authority movement is not an
optimization claim. Both medians remain far inside the 1 ms and 4 ms acceptance
budgets. Every authority run allocated zero times. Every rollback run retained
120,083 allocations / 142,279,564 requested bytes, exact depth 12, authority
history high-water 128, and rollback snapshot/input high-water 64/64.

The immutable before executable SHA-256 is
`f1409f734692e2d565faab416bf110628705dc45a9459a1101e0f53636c1fde1`;
the after executable SHA-256 is
`97d06e60f4e8becbf98071d489f8d02a9c44def9824a9751c4bf3b0e15bc0903`.
A prior valid non-alternating trio observed 68,083/434,042 ns before and
58,042/398,791 ns after for authority/rollback median p99; those corroborating
...
### Simulation-v10 Chick stamina-cost preservation capture

This comparison covers the authoritative ultimate admission/payment path and
Chick's authored reduced stamina costs. The immutable before source is accepted
simulation-v9 commit `c9a6696`; the after executable contains simulation v10
and BF032. Both arm64 executables used rustc 1.94.1, the profiling profile, seed
`0x00000000ffc00001`, 256 authority warmup ticks, 16 rollback warmup bursts,
1,000 timed samples, and exact rollback depth 12. The machine remained on AC
power under High Power mode with no fixed affinity.
...
| Pair | Before authority p99 (ns) | After authority p99 (ns) | Before rollback p99 (ns) | After rollback p99 (ns) |
| ---: | ---: | ---: | ---: | ---: |
| 1 | 855,709 | 896,625 | 407,083 | 418,417 |
| 2 | 868,750 | 874,458 | 445,083 | 402,833 |
| 3 | 872,959 | 869,542 | 404,375 | 405,791 |
| Median | 868,750 | 874,458 | 407,083 | 405,791 |

Authority p99 changed by +0.7%, exact rollback p99 by -0.3%, and bot-planner
p95 by -1.0% (37,292 to 36,916 ns). All are immaterial against the 1 ms,
4 ms, and 0.1 ms gates. Every authority run allocated zero times; every
rollback run retained 118,586 allocations / 142,135,996 requested bytes, exact
depth 12, authority history high-water 128, and rollback snapshot/input
high-water 64/64.

The immutable before executable SHA-256 is
`99d3c46f04a7dda3cb84e3b650621ba0cce5aa8644277b06dc76947750a24d90`;
the after executable SHA-256 is
`8b5ecfd1703445a0916990a758ed8c09aff21c57fdc67a7c2cf757ae340f4502`.
The six accepted records are checked in as
[raw JSON](performance-evidence/2026-08-27-chick-stamina.jsonl).
