...
| Native Steam platform, lobby, authentication and listen P2P/SDR | **Implemented + external validation pending** | `steam_platform.rs`, `steam_control.rs`, `network_quality.rs`, `online_roster.rs`, `online_lobby.rs`, `native_online.rs`, and the custom auth-gated `steam_transport.rs` own schema 4, AFCP v2, callback pumping, metadata, invites/launch joins, rich presence, direct and routed recipient-bound tickets, full-roster account authentication, semantic ACK quarantine/promotion, P2P sockets, relay/certificate initialization, quality, precommit RTT calibration and reconnect. The physical topology remains an explicit authority star. Relay/auth preparation and connection/auth setup are independently bounded to 15 seconds. | Hostile codec and transport fixtures cover ACK generation/bounds, semantic accept/reject, duplicate/stale frames, receive arming, timeout/retry and zeroization. The four-core fixtures prove exactly three physical links for four accounts, role-aware secure-link counts, three verified remote accounts per participant, routed client-to-client ticket validation without invented socket mappings, batched Ready order, an immutable participant/generation manifest transaction, `CommitAccepted`, receive-only activation receipts, an irrevocable reliable final release, scoped partial-activation cancellation, and exact endpoint counts. Pair fixtures cover abort rollback, transient close during agreement, rematch, new `MatchId`/connection, retirement and stale-generation immunity. Each authenticated physical remote must contribute 20 valid readings to a connection-generation-specific 32-sample window; the authority commits the worst nearest-rank p95 as an immutable 2–6 tick delay and rejects incomplete, mismatched, or greater-than-12-tick rollback budgets. These tests cannot prove Valve backend behavior, actual relay selection, account licensing, overlay behavior, NAT traversal, or cross-machine teardown. |
...
- Schema-4/AFCP-v2 bootstrap over the explicit socket star: direct link auth,
  recipient-bound full-roster tickets routed through the authority, semantic ACK,
  immutable participant/generation manifest agreement, commit receipts, receive
  arming, and the final activation-receipt barrier.
- AppID ownership callbacks, auth revocation, sanitized failure display, and
  confirmation that the candidate's Steam tickets remain within the pinned
  1024-byte limit. Ticket chunking is deferred and must not be inferred from the
  routed-ticket protocol.
...
- A sealed release-candidate repeat/attachment of the locally passing schema-v6
  `FourBotStress`, `MapCycle100`, and `Soak10Minutes` timing/allocation matrix.
  The local matrix is complete on frozen patched profiling binaries, but is not
  evidence for a later binary or sealed candidate.
...
- Steam depot/package validation with the real AFC App ID. The physical Spacewar
  480 12/12 cold-start matrix and bidirectional 20-cycle runs are still pending;
  `scripts/validate_spacewar_acceptance.py` now rejects an incomplete matrix,
  countdowns over 30 seconds, build/commit/archive mismatch, unknown privacy-risk
  fields, or any nonzero cleanup count. It validates supplied records and is not
  itself physical evidence. Even a future pass is development evidence only and
  never shipping evidence.
