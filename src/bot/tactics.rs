...
use crate::canonical_math::{vec2_distance, vec2_dot, vec2_length, vec2_normalize_or_zero};
...
impl CombatForecastState {
    pub(super) fn distance(self) -> f32 {
        vec2_distance(self.bot_position, self.target_position)
    }
}
...
fn forecast_single(
    ...
    let predicted_distance = (distance
        + vec2_length(relative_velocity) * contact_tick as f32 * 0.05 * 0.35
    ...
    let facing_quality = vec2_dot(
        vec2_normalize_or_zero(state.bot_facing),
        vec2_normalize_or_zero(state.target_position - state.bot_position),
    )
    .clamp(-1.0, 1.0);
    ...
    match response {
        OpponentResponse::Attack => {
            ...
            let target_facing_quality = vec2_dot(
                vec2_normalize_or_zero(state.target_facing),
                vec2_normalize_or_zero(state.bot_position - state.target_position),
            )
            .clamp(0.0, 1.0);
...
pub(super) fn bounded_velocity(velocity: Vec3) -> Vec2 {
    ...
    if vec2_length(planar) > 12.0 {
        planar = vec2_normalize_or_zero(planar) * 12.0;
    }
    planar
}
...
pub(super) fn response_family(
    ...
    match perceived_action {
        ...
        FighterAction::Moving => {
            let away = vec2_normalize_or_zero(Vec2::new(
                opponent_position.x - bot_position.x,
                opponent_position.z - bot_position.z,
            ));
            if vec2_dot(opponent_velocity, away) > 0.75 {
                OpponentResponse::Retreat
            } else {
                OpponentResponse::Wait
            }
