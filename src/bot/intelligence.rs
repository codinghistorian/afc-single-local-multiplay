...
use super::{
    ...
    apply_edge_steering, arena_hazard_avoid_radius, arena_hazard_avoidance, bot_held_item_decision,
    bot_personality, bot_pickup_score, bot_range_band, bot_recovery_decision,
    bot_should_guard_threat, bot_should_jump_for_elevation, defensive_away_from, edge_danger,
    item_avoidance_radius, special_avoid_radius,
};
...
pub(super) fn build_world_snapshot(
    ...
    for (fighter, transform, action, motor, stats, character, style, equipment, contact) in fighters
    {
        ...
        snapshot.fighters[fighter.id] = Some(FighterSnapshot {
            ...
            recovery_remaining: active_prediction.map_or(0.0, |prediction| {
                (prediction.recover_at_ms as f32 / 1_000.0 - action.elapsed).max(0.0)
            }),
            active_prediction,
...
impl BotDecisionTrace {
    pub(crate) fn signature(self) -> u64 {
        ...
        let distance = (self.target_distance * 20.0).round().clamp(0.0, 255.0) as u64;
        ...
        let base = target
            ...
            | score << 37;
        let forecast = (self.forecast_score * 100.0)
            .round()
            .clamp(i16::MIN as f32, i16::MAX as f32) as i16 as u16 as u64;
        let bias = (self.learned_bias * 1_000.0)
            .round()
            .clamp(i16::MIN as f32, i16::MAX as f32) as i16 as u16 as u64;
        let mut signature = base
            ^ (self.phase as u64).wrapping_mul(0x9e37_79b9)
            ^ self.tactic.map_or(0xff, |tactic| tactic as u8) as u64 * 0x85eb_ca6b
            ^ (self.branch as u64) << 52
            ^ (self.plan_step as u64) << 57
            ^ forecast.rotate_left(17)
            ^ bias.rotate_left(41)
            ^ distance.rotate_left(9);
        for (index, prediction) in self.predicted_responses.into_iter().enumerate() {
            let probability = (prediction.probability * 1_000.0)
                .round()
                .clamp(0.0, 1_000.0) as u64;
            signature ^=
                ((prediction.response as u64) | probability << 4).rotate_left((index * 19) as u32);
        }
        signature
    }
}
...
impl BotRuntimeStore {
    ...
    #[cfg(feature = "perf")]
    pub(crate) fn planner_timing_ms(&self) -> Vec<f64> {
        ...
        for slot in &self.slots {
            samples.extend(
                slot.planner_timing_ns[..slot.planner_timing_len]
                    .iter()
                    .map(|nanoseconds| f64::from(*nanoseconds) / 1_000_000.0),
            );
        }
        samples
    }
...
pub(super) fn drive_bot(
    ...
    if profile.tactical_planning_enabled
        && tactical_dash_hold_expired(action.action, action.elapsed, slot.commitment.is_some())
    {
...
fn external_threat_cost(
    ...
    let mut cost = edge_danger(position) * 0.35;
    for fighter in snapshot.fighters.iter().flatten() {
        if fighter.id == bot_id
...
fn complete_tactic_plan(
            ...
            .and_then(|value| *value)
    {
        ...
        plan.accumulated.position_change = (edge_danger(target.position)
            - edge_danger(plan.target_origin))
            + (edge_danger(plan.origin) - edge_danger(bot.position));
...
fn apply_active_plan_step(
    ...
    slot.intent.movement = apply_edge_steering(position, movement.normalize_or_zero());
...
fn advance_active_tactic(
    ...
    if plan_is_invalid(PlanInvalidationInputs {
        ...
        unsafe_ground: edge_danger(position) > 0.9 || (step.requires_grounded && !motor.grounded),
        stamina: stats.stamina,
    ...
    if slot.commitment_rejected {
        ...
        slot.active_plan = Some(plan);
        return apply_active_plan_step(slot, position, target.position);
    }
    ...
    slot.active_plan = Some(plan);
    apply_active_plan_step(slot, position, target.position)
}
...
fn start_tactical_plan(
    ...
    let (bot_action_phase, mut bot_remaining) =
        forecast_action_phase(action.action, action.elapsed, bot_prediction);
    ...
    let phase = classify_phase(PhaseInputs {
        ...
        bot_edge_risk: edge_danger(position),
        target_edge_risk: edge_danger(target.position),
    });
    ...
    let state = CombatForecastState {
        ...
        target_velocity,
        bot_facing: Vec2::new(motor.facing.x, motor.facing.z).normalize_or_zero(),
        target_facing: Vec2::new(target.facing.x, target.facing.z).normalize_or_zero(),
        bot_action: action.action,
        target_action: perceived_action,
        bot_action_phase,
        target_action_phase,
...
fn sync_legacy_brain(brain: &mut BotBrain, slot: &BotRuntimeSlot) {
    ...
    brain.attack_timer =
        slot.attack_ready_tick.saturating_sub(slot.decision_tick) as f32 * DECISION_STEP;
    brain.dash_timer =
        slot.dash_ready_tick.saturating_sub(slot.decision_tick) as f32 * DECISION_STEP;
    brain.movement_plan_timer = slot.commitment.map_or(0.0, |commitment| {
        commitment.expires_tick.saturating_sub(slot.decision_tick) as f32 * DECISION_STEP
    });
