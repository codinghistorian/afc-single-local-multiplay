...
use crate::canonical_math::{
    vec2_distance_squared, vec2_length, vec2_length_squared, vec2_normalize_or_zero,
...
impl NavigationBlockers {
    ...
    fn blocks_segment(&self, from: Vec3, to: Vec3, fighter_radius: f32) -> bool {
        ...
        self.iter().any(|blocker| {
            ...
            let start_distance_squared = vec2_distance_squared(start, blocker.center);
            let end_distance_squared = vec2_distance_squared(end, blocker.center);
            ...
            vec2_distance_squared(start + segment * amount, blocker.center)
...
impl ArenaNavigationGraph {
    fn build(arena: &ArenaDefinition) -> Result<Self, &'static str> {
        ...
        for grid_z in 0..depth {
            for grid_x in 0..width {
                ...
                if vec2_length(Vec2::new(x, z)) > arena.ringout_radius {
                    continue;
                }
            ...
            for (direction_x, direction_z) in DASH_JUMP_DIRECTIONS {
                for distance in 2..=max_dash_jump_cells {
                    ...
                    let horizontal_distance =
                        vec2_length(Vec2::new(dx as f32, dz as f32)) * NAV_CELL_SIZE;
    ...
    fn nearest_node(&self, position: Vec3) -> Option<NavigationNodeId> {
        ...
        for (index, node) in self.nodes.iter().enumerate() {
            let distance_squared = vec3_distance_squared(node.position, position);
...
fn jump_segment_crosses_gap(arena: &ArenaDefinition, from: Vec3, to: Vec3) -> bool {
    let horizontal_distance = vec2_length(Vec2::new(to.x - from.x, to.z - from.z));
...
impl CachedNavigationRoute {
    ...
    fn advance_reached_waypoints(&mut self, graph: &ArenaNavigationGraph, from: Vec3, tick: u64) {
        while self.cursor < self.len {
            ...
            let distance = vec2_length(Vec2::new(waypoint.x - from.x, waypoint.z - from.z));
...
impl BotNavigationCache {
    ...
    pub(crate) fn next_direction(
        ...
        for _ in 0..2 {
            ...
            if !route.record_progress(vec2_length(flat), decision_tick) {
                route.invalidate();
                continue;
            }
            return Some(vec2_normalize_or_zero(flat));
        }
        None
    }
}
...
mod tests {
    use super::*;
    use crate::arena_defs::{SPLIT_CAUSEWAY_ARENA_INDEX, TRAINING_GROUND_ARENA_INDEX};
