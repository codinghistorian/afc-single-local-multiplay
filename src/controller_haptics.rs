...
use crate::components::{Controller, FighterAction, LocalInputAssignment};
...
const MAX_COMBAT_HAPTIC_CUES_PER_FRAME: usize = 64;
...
pub struct CombatHapticQueue {
    cues: ArrayVec<CombatHapticCue, MAX_COMBAT_HAPTIC_CUES_PER_FRAME>,
    dropped: u64,
}

impl CombatHapticQueue {
    pub fn push(&mut self, cue: CombatHapticCue) {
        if self.cues.try_push(cue).is_err() {
...
mod tests {
    ...
    #[test]
    fn combat_haptic_queue_rejects_newest_work_at_its_fixed_capacity() {
        let mut queue = CombatHapticQueue::default();
        for _ in 0..=MAX_COMBAT_HAPTIC_CUES_PER_FRAME {
        ...
        assert_eq!(queue.len(), MAX_COMBAT_HAPTIC_CUES_PER_FRAME);
