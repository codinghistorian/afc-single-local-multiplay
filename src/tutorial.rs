...
use crate::simulation::{ElapsedTicks, milliseconds_to_ticks_ceil, seconds_to_ticks_ceil};
...
#[cfg(all(not(feature = "native"), not(target_arch = "wasm32")))]
fn save_tutorial_progress_contents(_contents: &str) -> Result<(), String> {
    Ok(())
}
...
const BASICS_STEPS: &[TutorialStep] = &[
    ...
    lesson_step(
        ...
        "Acquire Pig, move away to break lock, then release Aim.",
...
const TUTORIAL_SETTLE_STABLE_TICKS: u32 = milliseconds_to_ticks_ceil(180);
#[cfg(test)]
const TUTORIAL_SETTLE_STABLE_SECONDS: f32 = TUTORIAL_SETTLE_STABLE_TICKS as f32 / 60.0;
...
pub struct TutorialObjectiveBaseline {
    ...
    pub lesson_item_entity: Option<SimEntityId>,
...
pub struct TutorialSession {
    ...
    pub completion_stable_ticks: ElapsedTicks,
    pub completion_saw_airborne: bool,
    pub completion_activation_seen: bool,
    pub success_elapsed: f32,
    pub final_exam_won: Option<bool>,
    pub dummy_script_ticks: ElapsedTicks,
...
impl Default for TutorialSession {
    fn default() -> Self {
        Self {
            ...
            completion_stable_ticks: ElapsedTicks::ZERO,
            completion_saw_airborne: false,
            completion_activation_seen: false,
            success_elapsed: 0.0,
            final_exam_won: None,
            dummy_script_ticks: ElapsedTicks::ZERO,
            return_setup: None,
        }
    }
}
...
impl TutorialSession {
    ...
    pub fn reset_completion(&mut self) {
        ...
        self.completion_stable_ticks.reset();
        self.completion_saw_airborne = false;
        self.completion_activation_seen = false;
...
fn tutorial_settlement_status(session: &TutorialSession) -> Option<&'static str> {
    ...
    Some(match session.current_step()?.objective {
        ...
        TutorialObjective::AimUnlock => "RELEASE AIM TO FINISH",
...
pub fn configure_tutorial_match(
    ...
    active_arena.select(TUTORIAL_ARENA_INDEX);
    ...
    *camera_effects = CameraActionEffects::default();
    ...
    ) in &mut fighters
    {
        ...
        *drunk = DrunkStatus::default();
        ...
        if let Some(target_position) = positions.get(fighter.id) {
            position.translation = *target_position;
            fighter_position_floor_fix(&mut position);
            if let Some(mut transform) = render_transform {
                transform.translation = position.translation;
            }
            if fighter.id == TUTORIAL_PLAYER_ID {
                player_position = position.translation;
            }
        }
        *aim = FighterAimState {
    ...
    let mut item_order = items
        .iter()
        .map(|(entity, stable, ..)| (stable.id(), entity))
        .collect::<Vec<_>>();
    item_order.sort_unstable_by_key(|(stable, _)| *stable);
    let item_count = item_order.len();
    for (index, (_, entity)) in item_order.into_iter().enumerate() {
        let Ok((_, stable, mut item, mut transform, mut visibility, mut material, mut scene_root)) =
        ...
        if let Some((kind, position, phase)) = target {
            ...
            *visibility = Visibility::Visible;
            if !chapter.final_exam {
                ...
                lesson_item_entity = Some(stable.id());
            }
        } else {
    ...
    session.objective_baseline = TutorialObjectiveBaseline {
        ...
        aim_unlock_count: fighters
            .iter()
            .find_map(|(fighter, _, _, _, aim, _, _, _, _, _, _, _, _, _)| {
    ...
    session.dummy_script_ticks.reset();
...
fn fighter_position_floor_fix(position: &mut SimPosition) {
    position.translation.y = ARENA_TOP_Y;
}
...
pub fn script_tutorial_dummy(
    ...
    session.dummy_script_ticks.advance();
    let elapsed = session.dummy_script_ticks;
    for (fighter, controller, action, mut input) in &mut dummy {
        ...
        *input = FighterInput::default();
        match step.dummy {
            ...
            ScriptedDummyMode::TimedLightAttacks => {
                input.light = scripted_input_pulse(elapsed, 132)
                    || matches!(
                        action.action,
            ...
            ScriptedDummyMode::TimedHeavyAttacks | ScriptedDummyMode::KnockdownSetup => {
                input.heavy = scripted_input_pulse(elapsed, 168);
            }
            ScriptedDummyMode::NormalBot => {}
        }
    }
...
fn scripted_input_pulse(elapsed: ElapsedTicks, period_ticks: u32) -> bool {
    elapsed.get() % period_ticks < seconds_to_ticks_ceil(0.12)
}
...
pub fn observe_tutorial_objective(
    ...
    let dummy_state = fighters
        .iter()
        .find_map(|(fighter, _, _, action, _, motor, position, _)| {
            (fighter.id == TUTORIAL_DUMMY_ID).then_some((action, motor, position.translation))
        });
    let Some((input, aim, action, _stats, motor, position, inventory)) = fighters.iter().find_map(
        |(fighter, input, aim, action, stats, motor, position, inventory)| {
            (fighter.id == TUTORIAL_PLAYER_ID)
                .then_some((input, aim, action, stats, motor, position, inventory))
        },
    ) else {
        return;
    };
    ...
    let evidence_detected = match step.objective {
        ...
        TutorialObjective::Movement {
            ...
            let displacement = (position.translation
                - session.objective_baseline.last_player_position)
                .xz()
                .length();
            session.objective_baseline.last_player_position = position.translation;
        ...
        TutorialObjective::Guarding { count } => {
            if latch_event(
                motor.guard_counter_window_timer.active(),
        ...
        TutorialObjective::ItemUse { kind, uses } => {
            ...
            if let Some((stable, item)) = relevant {
                session.item_was_held |= inventory.held == Some(stable.id());
        ...
        TutorialObjective::ItemThrow(kind) => items.iter().any(|(stable, item)| {
            session.item_was_held |= inventory.held == Some(stable.id()) && item.kind == kind;
            item.kind == kind
        ...
        TutorialObjective::SpecialSpawn(kind) => specials.iter().any(|special| {
            special.owner == crate::determinism::FighterId::ZERO && special.kind == kind
        }),
        TutorialObjective::RingOutOpponent => {
    ...
    if session.completion_state == TutorialCompletionState::Observing && evidence_detected {
        ...
        session.completion_stable_ticks.reset();
        session.completion_saw_airborne = !motor.grounded;
        session.completion_activation_seen = false;
    }
    ...
    match step.objective {
        TutorialObjective::ItemThrow(ItemKind::Barrel) => {
            session.completion_activation_seen |= items.iter().any(|(_, item)| {
                item.kind == ItemKind::Barrel && matches!(item.state, ItemState::Spraying { .. })
            });
        }
        TutorialObjective::ItemThrow(ItemKind::Crate) => {
            let lesson_item_finished = session
                .objective_baseline
                .lesson_item_entity
                .and_then(|lesson_id| items.iter().find(|(stable, _)| stable.id() == lesson_id))
            ...
            let reward_revealed = items.iter().count() > session.objective_baseline.item_count
                && items.iter().any(|(stable, item)| {
                    Some(stable.id()) != session.objective_baseline.lesson_item_entity
                        && !matches!(item.state, ItemState::Respawning)
                });
            session.completion_activation_seen |= lesson_item_finished && reward_revealed;
        }
        TutorialObjective::ItemThrow(ItemKind::Steamer) => {
            session.completion_activation_seen |= session
                .objective_baseline
                .lesson_item_entity
                .and_then(|lesson_id| items.iter().find(|(stable, _)| stable.id() == lesson_id))
                .is_some_and(|(_, item)| matches!(item.state, ItemState::Respawning));
        ...
        TutorialObjective::ItemThrow(_) => {
            session.completion_activation_seen = true;
        }
        TutorialObjective::SpecialSpawn(kind) => {
            session.completion_activation_seen |= specials.iter().any(|special| {
    ...
    let settled = match step.objective {
        ...
        TutorialObjective::AimUnlock => !input.aim && player_recovered,
        TutorialObjective::Action(FighterAction::Jumping) => {
        ...
        TutorialObjective::ItemThrow(_) | TutorialObjective::SpecialSpawn(_) => {
            player_recovered && session.completion_activation_seen
        }
    ...
    if settled {
        session.completion_stable_ticks.advance();
    } else {
        session.completion_stable_ticks.reset();
    }
    if session.completion_stable_ticks.get() >= TUTORIAL_SETTLE_STABLE_TICKS {
...
fn tutorial_special_activation_matches(
    ...
    owner == FighterId::ZERO && actual_kind == expected_kind && active_feedback_sent
}
...
mod tests {
    ...
    /// Observer tests install the production fixed-tick system in `Update` so
    /// they can drive an exact number of canonical steps without a renderer.
    /// Each call site performs the final `app.update()` after this helper.
    fn advance_virtual_time(app: &mut App, seconds: f32) {
        let ticks = seconds_to_ticks_ceil(seconds);
        for _ in 1..ticks {
            app.update();
        }
    }
    ...
    fn item_objective_test_app(
        step_index: usize,
        kind: ItemKind,
        item_state: impl FnOnce(FighterId) -> ItemState,
    ) -> (App, Entity) {
        let mut app = App::new();
        ...
        item.state = item_state(FighterId::ZERO);
        let item_id = SimEntityId::new(SimEntityKind::Item, 0, 0);
        let item_entity = app
            .world_mut()
            .spawn((StableSimEntity::new(item_id), item))
            .id();
        {
            let mut session = app.world_mut().resource_mut::<TutorialSession>();
            session.objective_baseline.lesson_item_entity = Some(item_id);
    ...
    fn movement_objective_waits_for_input_release_before_success() {
        ...
        let player = app
            ...
            .spawn((
                ...
                FighterMotor::default(),
                FighterGrabState::default(),
                SimPosition::new(Vec3::new(-3.0, 0.0, 0.0)),
                FighterInventory::default(),
            ))
            .id();
    ...
    #[test]
    fn aim_objective_requires_acquire_then_manual_break_release_and_fixed_tick_settle() {
        let mut app = App::new();
        let mut session = TutorialSession::default();
        session.start(TutorialChapterId::Basics);
        ...
        app.insert_resource(session)
            ...
            .insert_resource(GameTransition::default())
            .insert_resource(Time::<()>::default())
            .add_systems(FixedUpdate, observe_tutorial_objective);
        let player = app
            .world_mut()
            .spawn((
                Fighter {
                ...
                FighterAimState {
                    locked_target: Some(FighterId::ALL[1]),
                    ..default()
                },
                FighterActionState::default(),
                FighterStats::default(),
        ...
        assert_eq!(
            app.world().resource::<TutorialSession>().phase,
            TutorialPhase::Playing,
            "release alone must wait for the fixed-tick settlement window"
        );

        for _ in 1..TUTORIAL_SETTLE_STABLE_TICKS {
            app.world_mut().run_schedule(FixedUpdate);
        }
        assert_eq!(
            app.world().resource::<TutorialSession>().phase,
    ...
    #[test]
    fn barrel_waits_for_the_canonical_spray_activation() {
        let (mut app, item_entity) =
            item_objective_test_app(5, ItemKind::Barrel, |owner| ItemState::Thrown {
                owner,
                lifetime: TickTimer::from_seconds_ceil(4.0),
                grace: TickTimer::ZERO,
            });
        ...
        assert_eq!(
            app.world().resource::<TutorialSession>().phase,
            TutorialPhase::Playing,
            "throw recovery alone must not complete the Barrel lesson"
        );
    ...
    fn mystery_crate_waits_until_its_reward_is_revealed() {
        ...
        app.world_mut()
            ...
            .state = ItemState::Respawning;
        app.world_mut().spawn((
            StableSimEntity::new(SimEntityId::new(SimEntityKind::Item, 1, 0)),
            ArenaItem::new(ItemKind::Apple, Vec3::ZERO, 0.0),
        ));
        advance_virtual_time(&mut app, TUTORIAL_SETTLE_STABLE_SECONDS);
        app.update();
        assert_eq!(
    ...
    fn fighter_recovery_rejects_active_air_and_dash_states() {
        ...
        assert!(!tutorial_fighter_has_recovered(&action, &motor));
        motor.grounded = true;
        motor.dash_slide_timer = TickTimer::from_ticks(1);
        assert!(!tutorial_fighter_has_recovered(&action, &motor));
    }
    ...
    fn special_spawn_waits_for_the_expected_active_effect() {
        ...
        assert!(!tutorial_special_activation_matches(
            FighterId::from_index(TUTORIAL_DUMMY_ID).unwrap(),
            SpecialKind::Projectile,
            true,
            SpecialKind::Projectile,
        ));
    ...
    fn guarding_objective_requires_a_confirmed_guard_impact() {
        ...
        app.world_mut()
            ...
            .unwrap()
            .guard_counter_window_timer = TickTimer::from_ticks(1);
        app.update();
    ...
    fn cleanup_restores_setup_but_keeps_the_tutorial_player_assignment() {
        ...
        let mut state = MatchState::default();
        let mut active_arena = ActiveArena::default();
        ...
        assert_eq!(active_arena.index(), setup.arena_index);
    ...
    fn scripted_dummy_pulses_are_timed_and_bounded() {
        assert!(scripted_input_pulse(ElapsedTicks::ZERO, 132));
        assert!(scripted_input_pulse(ElapsedTicks::from_ticks(135), 132));
        assert!(!scripted_input_pulse(ElapsedTicks::from_ticks(12), 132));
        assert!(!scripted_input_pulse(ElapsedTicks::from_ticks(131), 132));
    }
