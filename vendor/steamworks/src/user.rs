...
/// Called when the Steam client IPC channel fails.
///
/// The payload is intentionally opaque. Steam documents this callback as a
/// process-level loss of the client IPC channel, so applications should reset
/// their Steam-backed state instead of attempting to attribute it to a peer.
#[derive(Clone, Debug)]
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
pub struct IpcFailure;

impl_callback!(_cb: IPCFailure_t => IpcFailure {
    Self
});
...
pub enum AuthSessionValidateError {
    ...
    PublisherIssuedBan,
    /// The networking identity of the authenticator does not match the
    /// identity the ticket was issued for.
