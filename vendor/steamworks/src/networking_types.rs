...
pub enum NetworkingConfigValue {
    ...
    /// [connection int64] Arbitrary application data attached atomically when
    /// a connection is created. The value is included in connection status
    /// callbacks and inherited callback observations for that connection.
    ConnectionUserData,
...
impl NetworkingConfigValue {
    pub fn data_type(&self) -> NetworkingConfigDataType {
        match self {
            ...
            NetworkingConfigValue::ConnectionUserData => NetworkingConfigDataType::Int64,
...
impl From<NetworkingConfigValue> for sys::ESteamNetworkingConfigValue {
    fn from(value: NetworkingConfigValue) -> steamworks_sys::ESteamNetworkingConfigValue {
        match value {
            ...
            NetworkingConfigValue::ConnectionUserData => sys::ESteamNetworkingConfigValue::k_ESteamNetworkingConfig_ConnectionUserData,
...
impl AppNetConnectionEnd {
    ...
    pub fn normal(code: i32) -> Self {
        ...
        if code < min || code > max {
            panic!(
                "app net connection end code {} is out of range. App normal codes must be between {} and {}",
                code, min, max
            );
        }
    ...
    pub fn exception(code: i32) -> Self {
        ...
        if code < min || code > max {
            panic!(
                "app net connection end code {} is out of range. App exceptions code must be between {} and {}",
                code, min, max
            );
        }
...
impl ConnectionRequest {
    ...
    /// Queries the connection's current application userdata. Unlike
    /// [`Self::user_data`], which is the value captured when this callback was
    /// queued, this reads the live connection handle.
    pub fn connection_user_data(&self) -> Result<i64, crate::networking_sockets::InvalidHandle> {
        self.connection.connection_user_data()
    }

    /// Tags an incoming connection before it is accepted. This is useful when
    /// callbacks already in Steam's queue must be correlated with an
    /// application-owned connection generation.
    pub fn set_connection_user_data(
        &self,
        user_data: i64,
    ) -> Result<(), crate::networking_sockets::InvalidHandle> {
        self.connection.set_connection_user_data(user_data)
    }
...
impl NetworkingIdentity {
    ...
    pub fn steam_id(&self) -> Option<SteamId> {
        unsafe {
            ...
            if id == 0 { None } else { Some(SteamId(id)) }
        }
    }
