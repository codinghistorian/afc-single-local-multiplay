...
impl<R: RecordDiagnostics + ?Sized, P: Pass> PassSpanGuard<'_, R, P> {
    ...
    pub fn end(mut self, pass: &mut P) {
        self.recorder.end_pass_span(pass);
        // AFC backport: `name` can be an owned shadow-pass label. Release it
        // before forgetting the guard, while preserving Bevy's guard contract.
        drop(core::mem::replace(&mut self.name, Cow::Borrowed("")));
        core::mem::forget(self);
    }
}
